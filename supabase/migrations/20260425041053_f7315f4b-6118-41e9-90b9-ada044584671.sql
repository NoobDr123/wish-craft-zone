...
ADD COLUMN IF NOT EXISTS delivery_tier text NOT NULL DEFAULT 'standard'
CHECK (delivery_tier IN ('standard', 'express_48h', 'rush_24h'));

CREATE INDEX IF NOT EXISTS idx_orders_delivery_tier ON public.orders(delivery_tier);

-- Backfill any existing rows that were rush=true to rush_24h
UPDATE public.orders
SET delivery_tier = 'rush_24h'
WHERE is_rush = true AND delivery_tier = 'standard';

-- Allow ordinary users to keep updating their orders without tripping
-- the tamper-protection trigger on this new column. Service role / admins
-- still set it; users never can.
