-- 1) Replace the INSERT RLS policy with a strict one matching real checkout behavior.
...
-- 2) Extend the tamper-protection trigger to ALSO run BEFORE INSERT as a backstop.
--    Service role and admins always pass through. Buyers can only create unpaid rows.
...
BEGIN
  ...
  -- ===== INSERT path: enforce safe defaults =====
