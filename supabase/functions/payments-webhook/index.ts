...
async function sendOrderConfirmation(orderId: string) {
  // Atomic claim — only one of the racing callers (payments-webhook,
  // confirm-payment, mark-upsells-complete) wins and actually sends.
  ...
  if (!res.ok) {
    ...
    // Roll back the claim so another path can retry.
    await supabase
      .from("orders")
